...
\leongage{} binaries have the extension \lstinline$.no$ (from \leongage{} object file)  and consist of three segments, a header segment, a string storage segment and a program segment.

The header segment takes the first six bytes and contains a file identifier (the digits 1 and 7, each stored in one byte) followed by the file format version. This version information starts with the character \lstinline$'v'$ followed by three bytes. Figure~\ref{fig:binaryfileformat} shows an example of such a file. From bytes 3 to 5 it can be seen that this is a file in version 1.0.0.

The string storage segment starts with a four byte number $s$ holding the length of the string storage segment and then follows a bunch of $s$ bytes holding the characters of the strings stored.

The final program segment is organized analogously to the string storage segment. Again a four byte number $p$ is followed by $p$ bytes of program code.
...
\cline{2-7} 
0 & 1 & 7 & 'v' & 1 & 0 & 0 \\ \cline{2-7}
6 & \multicolumn{4}{|c|}{$s$} & \multicolumn{2}{c|}{ } \\ \cline{2-5}
12 & \multicolumn{6}{|c|}{ } \\
 & \multicolumn{6}{|c|}{$s$ bytes of } \\
 & \multicolumn{6}{|c|}{ strings in the } \\
 & \multicolumn{6}{|c|}{ string storage segment} \\
 & \multicolumn{6}{|c|}{ } \\ \cline{2-7}
$10+s$ & \multicolumn{4}{|c|}{$p$} & \multicolumn{2}{c|}{ } \\ \cline{2-5}
 & \multicolumn{6}{|c|}{ } \\
 & \multicolumn{6}{|c|}{$p$ bytes of } \\
 & \multicolumn{6}{|c|}{ program code in the} \\
 & \multicolumn{6}{|c|}{ program segment } \\
 & \multicolumn{6}{|c|}{ } \\ \cline{2-7}
\end{tabular}
\end{center}
